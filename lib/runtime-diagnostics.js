...
class RuntimeDiagnostics {
  ...
  readRecent(limit = this.config.viewerEntries) {
    ...
    // Newest file first, stopping once the window is full. The store grows without
    // bound, so reading all of it to return a small tail costs more every day and
    // stalls the bridge that is doing the reading.
    const chunks = [];
    let collected = 0;
    for (let index = files.length - 1; index >= 0; index -= 1) {
      const records = readDiagnosticRecords(files[index].filePath);
      chunks.unshift(records);
      collected += records.length;
      if (maximum > 0 && collected >= maximum) {
        break;
      } else {
        // Older files are still needed to fill the window.
      }
    }
...
function readDiagnosticRecords(filePath) {
  const records = [];
