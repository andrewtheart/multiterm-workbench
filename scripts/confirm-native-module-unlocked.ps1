...
function Confirm-MultiTermNativeModuleUnlocked {
    ...
    $answer = (Read-Host 'Stop the identified MultiTerm process(es) and continue? (yes/no)').Trim()
    if ($answer -ine 'yes') {
        throw 'Native rebuild cancelled; the running MultiTerm process was left untouched.'
    }

    $orderedProcesses = @($blockingProcesses | Sort-Object @{
        Expression = { if ($_.Role -eq 'MultiTerm Electron owner') { 0 } else { 1 } }
    }, Id)
    foreach ($blockingProcess in $orderedProcesses) {
        if (Get-Process -Id $blockingProcess.Id -ErrorAction SilentlyContinue) {
    ...
    $remainingProcesses = @(Get-MultiTermNativeProcess -Root $Root -NativeModulePath $NativeModulePath)
    if ($remainingProcesses.Count -gt 0) {
        $remainingIds = @($remainingProcesses | ForEach-Object { $_.Id } | Sort-Object -Unique) -join ', '
        throw "conpty.node is still in use by MultiTerm process PID(s): $remainingIds. Close them and retry."
    }
