...
Two round controls sit at the right edge of every pane: one near the top scrolls to the oldest retained line, one near the bottom returns to the newest. Both brighten while the pane is away from that end. In a Copilot CLI session, which owns its own scrollback, they drive Copilot's scrolling instead and hide themselves in other full-screen programs that keep no scrollback.

In a Copilot CLI prompt box, hold <kbd>Shift</kbd> and press <kbd>Left</kbd> or <kbd>Right</kbd> to highlight text, and <kbd>Ctrl</kbd>+<kbd>A</kbd> to highlight everything in that prompt box only. <kbd>Delete</kbd> or <kbd>Backspace</kbd> then removes the highlighted text. Any other key cancels the highlight. Copilot's own prompt box has no text selection, so MultiTerm tracks it alongside Copilot's cursor and draws the highlight itself; outside a Copilot prompt box every one of these keys keeps its normal terminal meaning, including <kbd>Ctrl</kbd>+<kbd>A</kbd> selecting the whole terminal.
...
Choose **Change appearance...** in the terminal context menu to open the tabbed appearance editor. Its **Terminal** section provides compact inline 24-bit pickers for background and font colors: drag in the saturation/brightness field, adjust the hue rail, or enter a hex value or independent 0-255 red, green, and blue channels. Use the expand button beside a hue rail when a larger picker would help. The Settings and Terminal Appearance font selectors share a 20-family catalog, and every menu row is rendered in the face it represents. The terminal preview updates live. Select **Apply** to reveal **This terminal** and **All terminals** directly beneath it, then choose where to commit the preview. Per-terminal choices survive renderer reloads, terminal restarts, and saved workspace restoration.

Choose **Header background...** to open the same editor directly on its **Header** section. The **palette** button beside that terminal's bell remains the quick route: it opens a small flyout centered under the button with one-click colors, **Your colors**, **Use default**, and **More options...** for the shared full editor. Select **+** under **Your colors** to open the native color picker and add one reusable color at a time, up to six. Each saved swatch applies its gradient with one click and has an **x** button to remove only that color; removing one restores **+**. MultiTerm never evicts a saved color to make room for another, and choosing a color that already exists or is already a preset does not consume another slot. The Header section chooses between **Solid** and **Gradient**. Solid offers an eight-color palette; Gradient supports linear, radial, and conic gradients with adjustable angle or center, two to eight color stops, arbitrary colors, opacity, and stop positions. **Font face** and **Font size (px)** restyle only the pane title; leave the size empty to keep the app default, and values are held between 9 and 20 px. Your edits preview live on the pane header itself; **Apply** stores the selected scope, **Use default header** removes its override, and **Cancel** restores the saved background. Elevated and awaiting-input headers keep their warning colors, which take precedence over a custom background. The separate **Cycle color** action still controls the pane's label accent.
...
When the picker opens a new terminal, each result also carries a checkbox for opening several sessions at once. Selecting one or more reveals **Review selected**, which lists every chosen session with an editable working directory. MultiTerm validates each directory before it opens anything, marks the ones that need attention, and reports separately when the bridge did not answer rather than blaming the folder. **Open selected sessions** then launches them in order, and any that fail stay selected for a retry. Clicking a card itself still resumes only that one session.

**Session list timeout (s)** under Session controls how long MultiTerm waits for the first local Copilot or Claude session list; the slower VS Code and Visual Studio history pass continues separately in the background. If the bridge does not answer, the picker keeps **Refresh** available, and a silent provider recheck retains the last known assistant availability instead of disabling Resume. A successful provider response that reports no available assistant still disables unavailable actions normally.
...
Session cards also show the terminal notes belonging to that session, so you can tell two similar sessions apart before resuming either. The Copilot CLI never reports the ID of a session it starts, so MultiTerm supplies one with `--session-id` whenever it launches Copilot; that ID is what links a terminal's notes to the session. Because the link is an ID rather than a name, renaming a terminal never breaks it. Notes from the still-open terminal appear first, followed by the notes recovered from each terminal that has since ended. Each card lists one page of notes at a time and trims long ones to a preview: **Show more...** reveals the next page, and once every note is on screen it reveals their full text, while **Show less** returns the card to a single page. Expanding never resumes the session. **Session notes per page** under Session sets how many notes one page holds, from 1 to 50, and defaults to 5. Sessions started before this existed, or started outside MultiTerm, have no recorded ID, so their notes are matched by working directory instead and the card labels them **Matched by folder**.

Hover a Copilot session card, or move keyboard focus to it, to see terminal titles recorded for that exact assistant session. The list includes the current or last recorded title, titles entered manually in MultiTerm, and AI title suggestions marked **Accepted**, **Not accepted**, or **Awaiting decision**, together with the terminal name and PID when available. Right-click the session card to expose the same title history alongside Resume/Connect and applicable worktree actions. Exact session identity is preferred; terminal ID or PID is used only to backfill older title records that did not yet store a session key. A title or shared working directory alone never links histories. Existing AI suggestion records can appear when that exact terminal identity is still known; manual title history begins with this version because earlier manual renames were not recorded. **History entries** under AI assistants controls retention for both suggestions and manual terminal titles.
...
Terminals whose working directory sits inside a Git repository show a **branch** button in the pane header, badged when that branch has uncommitted work. Select it, or press <kbd>Ctrl+Alt+Shift+G</kbd>, to open **Review changes**. The same dialog opens from **Review** in the worktree manager and from a linked-worktree session card.

The dialog lists every change in a **Staged** and an **Unstaged** pane with per-kind icons, and shows the selected file's diff beside them. A key above the diff names every colour it paints: added and removed lines, the exact characters that changed within a line, unchanged context, the `@@` marker where untouched lines were skipped, and search matches. Each entry in the key also shows the prefix Git itself prints, so the diff stays readable without relying on colour alone. Stage or unstage a whole file with its inline button, by dragging the row between panes, or with <kbd>S</kbd> and <kbd>U</kbd> while the file list has focus. **Stage all** and **Unstage all** act on everything at once. Each hunk header in the diff carries its own **Stage hunk** or **Unstage hunk** control, which moves only that hunk into or out of the index and never rewrites the file you are still editing. Untracked and conflicted files are staged whole, because a hunk cannot be applied to a file the index does not yet know about. Staging never sends you to the copy that just moved: the diff stays on the pane you are working through, on the same file while it still has changes left, then on the next file in that pane.

**Fill the window** in the dialog header expands Review changes to the whole viewport, giving the diff every spare pixel; the button restores the smaller card, and MultiTerm remembers which size you last used.

Every file row carries an **open** button, and each hunk header offers **Open line N**, which opens the file in your editor at that hunk rather than at the top. <kbd>O</kbd> opens the focused file while the file list has focus. Opening never stages anything. A deleted file has no working copy left, so its control is disabled. **Editor command** under **Git** chooses the program, defaulting to `code`; set it to `code-insiders`, `codium` or `cursor` to use those instead. It must be a bare command name on `PATH` — MultiTerm passes the file and line to it with `--goto` and never through a shell, so a file name containing `&` cannot become a second command.

Write the commit message in the box below, optionally starting from a saved template. **Suggest** asks GitHub Copilot for a message based on the staged diff and shows it for approval, so a message you are already typing is never overwritten; choose **Use**, **Edit**, or **Discard**. While Copilot is working, the status line names the stage it has reached and counts the seconds it has been waiting, and **Cancel** drops the request; cancelling ends the model call itself rather than just hiding the wait, and is not reported as a failure. The request is tool-free and non-persistent, and the diff is passed as untrusted data. **Preview commit** replaces the viewer with exactly what would be committed. **Commit staged** commits only the staged half of the working tree; it stays disabled until a message is present and every footer listed in **Required commit footers** appears.

**Push** sends the current branch to `origin`, creating the upstream on a first push, and reports its result inline with the branch's ahead/behind position. Terminal prompts are disabled for that push, so a stored credential helper answers silently but MultiTerm can never block on an invisible prompt. When Git needs credentials it cannot supply, **Run push in a terminal** hands the exact command to the terminal you opened the dialog from so you can answer it there.

**Merge...** reveals a target branch box, defaulting to the branch's recorded worktree parent, its upstream, or the repository's default branch. The box is a searchable list: type any part of a branch name to narrow the suggestions, which is quicker than scrolling a repository with many branches. Only a name that matches a branch exactly is accepted, so **Merge locally** and **Create pull request** stay disabled while a partly typed filter is showing. MultiTerm previews the merge base and whether the merge fast-forwards, merges cleanly, or conflicts, naming the conflicting files before anything runs. **Merge locally** performs the merge; conflicts open the same side-by-side resolver used for worktrees, showing base, current, and incoming with a live merged result, and returns to Review changes when every file is resolved. **Create pull request** uses the GitHub CLI when the remote is GitHub and `gh` is installed, and otherwise opens the forge's own comparison page in your browser.

Search the dialog with <kbd>Ctrl+F</kbd>. Matches are highlighted in the diff with a distinct active match, <kbd>Enter</kbd> and <kbd>Shift+Enter</kbd> move between them, and the file lists filter to the files that contain the query. The scope selector limits the search to staged changes, unstaged changes, or both. When a worktree is opened from the manager, a second scope selector switches the viewer between its **Pending changes** and a read-only **Branch vs parent** comparison.

Under **Git** in Settings, set how many files each pane lists, the largest diff to load, push and merge timeouts, the footers every commit message must carry, and the reusable commit message templates.
...
- Double-click a page name to rename it, or use its pencil button. In a left or right pages panel, a single click from the tab's leading edge through the first three title characters also starts renaming without switching pages.
...
- Press <kbd>Delete</kbd> with the page tabs focused to close the selected range, or the focused page when there is no range. With multiple tabs selected, the context menu adds **Close pages** alongside the existing single-page command. The batch uses the same **When closing a page with terminals** setting and asks once when configured to ask. Unselected pages are preserved; closing every page leaves one **Page 1**. Delete inside a page-name editor only edits the name.
...
- Give a group its own colour with the palette button at the end of its band. It opens the same picker a terminal's header background uses, with the same preset and saved custom colours, **Use default** to clear it, and **More options...** for the full gradient editor. The colour paints the group's label, so the page tabs inside keep their normal contrast, and it is saved with the group.
...
Choose **Command based automation** for ad-hoc shell commands, ad-hoc PowerShell, or `.ps1`, `.cmd`, and `.bat` script paths. Choose **Copilot automation** for scheduled Copilot CLI prompts. Choose **Copilot conditional automation** to have Copilot judge a condition on a schedule and act only when it is true. Choose **Appearance automation** to style terminals by title instead of running work. Create an interval, daily, or selected-weekday schedule, then build the workflow from connected visual step cards. Interval schedules also offer preset chips from 15 minutes to 24 hours.
...
Every step after the first has a graphical conditional gate. Select any prior steps, choose **all** or **any**, and run when those steps **succeed**, **fail**, **finish**, have output that **matches**, or have output that **does not match**. **Any** opens as soon as one selected, completed dependency satisfies the gate; **all** waits for every selected dependency. Output gates support **Contains**, **Exact line or output**, and **Regular expression** matching, independently set to case-sensitive or case-insensitive. With **Match across lines** off, each output line is tested separately; turn it on to test the complete multiline output and enable regex dot-all plus multiline-anchor behavior. Empty patterns, invalid regular expressions, and dependencies that were only staged without execution fail closed for both match and does-not-match gates.

Use the arrow controls to reorder cards and **Add another action** for an arbitrary-length chain. Command workflows emit hidden start and completion markers, so only text produced while the command runs is captured; the wrapper command and shell prompt cannot satisfy the predicate. Copilot prompts remain running until the TUI returns to its empty composer, then MultiTerm reads assistant-message records appended after that prompt's byte cursor in `~/.copilot/session-state/<session-id>/events.jsonl`. This avoids depending on the Copilot TUI screen, which does not expose complete scrollback. **Automation output capture (KB)** under **Performance** controls how much of each step's newest output is retained and bounds Copilot event reads (16-512 KB, default 128). **Automation step timeout (minutes)** bounds how long either task type may wait to start and finish (1-1440 minutes, default 30); a timeout fails the step so failure gates can run. A staged action is not submitted and therefore has no completion output to match.
...
A **Copilot conditional automation** runs a Copilot session on a schedule to judge a question you write in plain language, and carries out an action only when the answer is yes. Under **When**, describe the condition, for example `A new file has been written to D:\Incoming since the last check`. Under **Then**, describe what to do about it, for example `Delete the file`. A **Working directory** is required; Copilot starts there and, unless you widen the scope, can only reach that folder and its subfolders. The directory is checked again on every run, not only when you save, so a folder that disappears fails the check visibly instead of silently doing nothing somewhere else.
...
New profiles start with Settings collapsed to a 30-pixel rail, one-tenth of the normal 300-pixel panel width. Select the rail's expand button to open Settings. The open state and expanded width are remembered, so existing profiles keep their saved layout.
...
The UI reconnects automatically when the local bridge restarts. It also sends application-level heartbeats while the window is visible; if a connection remains technically open but the bridge stops answering, MultiTerm marks the terminals offline, replaces the stale socket, and reconnects. **Bridge response timeout** under **Performance** controls how long MultiTerm waits and also bounds installed-bridge WebSocket sends (10-300 seconds, default 30).
