...
- Open **Automations** from the workflow button to define interval, daily, or weekday schedules with ordered actions. Each action explicitly runs or stages without Enter after the named terminal reaches a confirmed shell/Copilot prompt. Drag the subtle output grip on a producer terminal to a consumer's input grip to create a directional handoff route and live arrow. Completed Copilot responses can emit `**HAND OFF** Terminal name` plus a payload; MultiTerm queues it in the bridge and stages it when that connected consumer is ready. An unnamed marker opens a same-page/CWD PowerShell terminal, launches `copilot --yolo`, and stages the context after Copilot becomes ready. Activity history, global pause, catch-up behavior, and history retention are visible in Automation Studio.
