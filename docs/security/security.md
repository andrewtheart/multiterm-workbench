...
Directional GUI links are renderer-owned visual/routing metadata stored in `multiterm.terminalLinks`. Loaded records are capped, require two distinct bridge-shaped session IDs, and are pruned unless both sessions are live. Link labels and route rows use DOM/text APIs. Message-created links remain visual only. A route created or upgraded through terminal-body grips carries `handoffEnabled:true`, allowing only the marker-driven readiness workflow above; it does not change bridge authorization or imply authenticated provenance.

### Scheduled automations

Schedule definitions and accepted pending stages are local profile data under `multiterm.automations`. Creating or editing a schedule ends with an explicit **Save & enable** action; every action visibly chooses **Run when ready** or **Stage when ready**. Run actions reuse the existing single-line `safeTerminalCommand` and automatic prompt-aware queue. Stage actions also pass `safeTerminalCommand`, persist before delivery, and paste without Enter after readiness. Targets resolve by exact case-insensitive visible terminal name; missing or duplicate names block instead of selecting arbitrarily.

One renderer owns a short renewable bridge lease while evaluating interval/daily/weekday schedules. Node serializes acquisition in its event loop; the installed host uses a dedicated lock, and both release ownership on renderer disconnect or after expiry. Before advancing a rule, the owner atomically claims its exact due timestamp; each bridge retains the latest claimed timestamp per rule, fencing a renderer that resumes after its lease expired. Queued actions carry the same occurrence identity so reload recovery deduplicates them. This prevents duplicate execution by multiple windows but is not a hostile-process security boundary. A same-user process that can reach the loopback bridge or modify the browser profile can also affect automation behavior. Global Pause, per-rule enabled state, catch-up policy, next-run text, and retained activity are visible in Automation Studio. History retention is user-configurable, including `0`; handoff payload size and inbox capacity reuse the existing visible Communication settings and bridge ceilings.
